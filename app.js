...
app.post('/login-register', async (req, res)=>{
    ...
    connection.query('INSERT INTO users SET ?', {users: usuarioR, correo:correoR, pass:passwordHaash}, async(error, results)=>{
        if(error){
            console.log(error);
        }else{
            res.render('login-register',{
...
app.post('/auth', async(req, res)=>{
    ...
    if (usuarioL && passwordL){
        connection.query('SELECT * FROM users WHERE users = ?', [usuarioL], async (error,results)=>{
